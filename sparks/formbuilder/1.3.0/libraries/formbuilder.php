...
class Formbuilder{
	...
	function open( $action, $multipart=FALSE, $params=array("class"=>"form-horizontal") )
	{
	...
	function text( $var, $label=null, $lblOptions=array("class"=>'control-label'), $fieldOptions=array("class"=>'input-xlarge') )
	{

		$fieldOptions['id'] = $this->_auto_id($fieldOptions, $var);

		$ret = $this->form_label( $label, $fieldOptions['id'], $lblOptions )."\n";
		$ret .= "<div class='controls'><input type=\"text\"  name=\"$var\" value=\"".$this->get_val( $var )."\"".$this->attribute_string( $fieldOptions )."/></div>";
	...
	public function submit( $var, $label, $options=array("class"=>"btn btn-primary") )
	{
		$atts = $this->attribute_string( $options );
		return "<div class='form-actions'><input type=\"submit\" name=\"$var\" value=\"$label\" $atts/></div>";
	}
